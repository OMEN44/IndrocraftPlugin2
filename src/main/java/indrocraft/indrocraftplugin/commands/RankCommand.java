...
public class RankCommand implements TabExecutor {
                        ...
                        case "delete":
                            if (args.length >= 2) {
                                if (rankExists(args[1])) {
                                    ...
                                    c.saveConfig();
                                    c.reloadConfig();
                                    player.sendMessage(ChatColor.BLUE + "Successfully deleted rank: " + ChatColor.GREEN
                                            + args[1]);
                                } else {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
            ...
            switch (args.length) {
                case 2:
                    ...
                    if (args[0].equalsIgnoreCase("delete"))
                        tabCompleter.addAll(Objects.requireNonNull(config.getConfigurationSection("ranks")).
                                getKeys(false));
        ...
        if (pre.toLowerCase().contains("nextrank:")) {
            completions.clear();
            for (String ranks : Objects.requireNonNull(config.getConfigurationSection("ranks")).
                    getKeys(false))
